...
const FeedbackDashboard = () => {
  ...
  const fetchLatestAnalysis = async () => {
    ...
    try {
      const {
        data: sessions,
        error
      } = await supabase.from('analysis_sessions').select('*').eq('user_id', user.id).eq('status', 'completed').order('created_at', {
        ascending: false
      }).limit(1);
      ...
      if (sessions && sessions.length > 0) {
        ...
        if (analysisDataObj?.frameAnalysis && analysisDataObj.frameAnalysis.length > 0) {
          ...
          processedFrames = analysisDataObj.frameAnalysis.map((frame: any, index: number) => ({
            ...
            // Include the actual frame image
  ...
  const fetchTechniqueStandards = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('technique_standards').select('*').eq('technique', 'bottom_turn').eq('wave_type', 'beach_break');
      if (error) throw error;
      setStandards(data || []);
    } catch (error) {
  ...
  const getIdealRange = (metricName: string, skillLevel: string) => {
    const standard = standards.find(s => s.metric_name.toLowerCase() === metricName.toLowerCase().replace(' ', '_') && s.skill_level === skillLevel);
    if (standard) {
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <Card className="mb-8 shadow-wave">
            ...
            <CardContent className="text-center">
              ...
              <p className="text-muted-foreground">
                {analysisData.overallScore >= 90 ? "Excellent technique!" : analysisData.overallScore >= 75 ? "Good technique! Keep practicing." : "Room for improvement. Focus on the highlighted areas."}
              </p>
            </CardContent>
          </Card>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            return <Card key={index} className="shadow-wave hover:shadow-depth transition-all duration-300">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        {metric.score >= 85 ? <TrendingUp className="h-5 w-5 text-accent" /> : <TrendingDown className="h-5 w-5 text-destructive" />}
                      </div>
          ...
          {/* Pose Visualization */}
          {frameData.length > 0 && <div className="mb-8">
              <h3 className="text-2xl font-bold mb-6">Frame-by-Frame Analysis</h3>
              <PoseVisualization frames={frameData} videoUrl={analysisData.videoUrl} />
            </div>}
          ...
          <div className="space-y-4">
            ...
            {analysisData.feedback.map((tip: string, index: number) => <Card key={index} className="shadow-wave hover:shadow-depth transition-all duration-300">
